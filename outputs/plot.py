...
if __name__ == '__main__':
    ...
    for filename in args.file:
        ...
        mean_fuel = moving_average(main_df.groupby('step_time').mean()['total_fuel_consumption'], window_size=args.window)
        #sem = moving_average(main_df.groupby('step_time').sem()['total_wait_time'], window_size=args.window)
        std_fuel = moving_average(main_df.groupby('step_time').std()['total_fuel_consumption'], window_size=args.window)
        ...
        ax1.set_ylabel('Total Waiting Time of Vehicles (s)', color=color)
        ax1.plot(steps, mean, color=color)
        
        ax1.fill_between(steps, mean + std, mean - std, alpha=0.3)
        ax1.tick_params(axis='y', labelcolor=color)

        ax2 = ax1.twinx()  # instantiate a second axes that shares the same x-axis

        color = 'tab:blue'
        ax2.set_ylabel('Total Fuel Consumption of Vehicles (s)', color=color)  # we already handled the x-label with ax1
        ax2.plot(steps, mean_fuel, color=color)
        
        ax2.fill_between(steps, mean_fuel + std_fuel, mean_fuel - std_fuel, alpha=0.3)
