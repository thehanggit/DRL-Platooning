...
if __name__ == '__main__':
    ...
    env = PlatoonEnvironment(net_file='../maps/SingleIntersection/corridor/corridor.net.xml',
                             route_file='../maps/SingleIntersection/corridor/corridor.rou.xml',
                             cfg_file='../maps/SingleIntersection/corridor/corridor.sumocfg',
                             use_gui=False,
                             direct_start=True,
                             num_seconds=400,
                             max_depart_delay=0,
                             ...
                             num_green_phases=2,
                             phases=[
                                 traci.trafficlight.Phase(8, "GGrr"),  # north-south
                                 traci.trafficlight.Phase(2, "yyrr"),
                                 traci.trafficlight.Phase(8, "rrGG"),  # west-east
    ...
    for episode in range(1, config.MAX_EPISODES + 1):
        ...
        sio.savemat(output_folder + 'RL_platoon_result.mat', {'R_history':R_history, 'Delay_history': Delay_history, 'Fuel_history':Fuel_history})
        ...
        out_csv3 = output_folder + 'action_episode{}.csv'.format(episode)
        # env.save_csv(out_csv, info['step_time'])
        env.save_csv2(out_csv2, info['step_time'])
        env.save_csv3(out_csv3, info['step_time'])
        env.close()
